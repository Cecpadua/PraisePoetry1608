...
def preprocess_image(image_path,border_size=10):
    # Read the image
    img = cv2.imread(image_path)

    # Add a white border
    height, width = img.shape[:2]
    bordered_img = np.ones((height + 2 * border_size, width + 2 * border_size, 3), dtype=np.uint8) * 255
    bordered_img[border_size:border_size + height, border_size:border_size + width] = img
...
def extract_text_from_image2(image_path):
    ocr = CnOcr(det_model_name='doc-', scale=1.2, gpu=False)
        ...
        for slide_number, slide in enumerate(presentation.slides, start=1):
            ...
            for shape_number, shape in enumerate(slide.shapes, start=1):
                ...
                if shape.shape_type == 13:  # 13 corresponds to Picture shape type
                    ...
                    text = extract_text_from_image(enhance_text_image_path).strip().replace(' ', '').replace('.', '')
                    print(f'[Result]: {text}')
